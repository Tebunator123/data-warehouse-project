...
Performance Analysis - Comparing the current value to a target value. 
					   Help us measure success and compare performance.
Formula : Current[Measure] - Target[Measure]
*/

/*
Analyze the yearly performance of products by comparing their sales 
to the average sales performance of the product and the previous year's sales
*/
...
CASE WHEN current_sales - AVG(current_sales) OVER (PARTITION BY product_name) > 0 THEN 'Above Avg'
	 ...
	 ELSE 'Avg'
END AS avg_change,
LAG(current_sales) OVER (PARTITION BY product_name ORDER BY order_year) AS previous_year_sales, -- to access previous value of current_sales
